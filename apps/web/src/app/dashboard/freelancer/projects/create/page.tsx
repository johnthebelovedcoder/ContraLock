...
export default function FreelancerCreateProjectPage() {
  ...
  if (!isAuthenticated || user?.role !== 'freelancer') {
    return (
      <div className="flex items-center justify-center h-[calc(100vh-200px)]">
        <p className="text-muted-foreground">Please log in as a freelancer to create a project</p>
      </div>
    );
  }
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        <div>
          ...
          />
          <p className="text-xs text-muted-foreground mt-1">
            The client will receive an invitation to review this project
          </p>
        </div>
      </CardContent>
    </Card>
                ...
                <CardContent>
                  <div className="space-y-3">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">{projectData.clientEmail}</span>
                    </div>
                  </div>
                </CardContent>
              </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-3">
                ...
                <p>
                  After submission, the client will receive an invitation to review your project.
                  They will need to accept the project before funding can be processed.
                </p>
              </div>
            </CardContent>
          </Card>
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const projectToCreate: Omit<Project, 'id' | 'clientId' | 'createdAt' | 'updatedAt' | 'milestones'> & { milestones: any[] } = {
        ...
        freelancerId: user?.id, // Freelancer is creating this project
        clientEmail: projectData.clientEmail, // Store client email to invite them
      ...
      alert('Project created successfully! Client has been invited to review the project.');
      setCurrentStep(1);
      setProjectData({
        title: '',
        description: '',
  ...
  const nextStep = () => {
    if (currentStep === 1) {
      if (!projectData.title || !projectData.category || projectData.totalBudget <= 0 || !projectData.description || !projectData.clientEmail) {
    ...
    <div className="space-y-6 p-4">
      <div>
        ...
        <p className="text-muted-foreground">
          Create a project as a freelancer and invite your client to review and fund it
        </p>
      </div>
